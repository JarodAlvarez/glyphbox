...
local lp,lt,bp,bt=1,-1,1,-1
local function gb(s,i) local b=(i-1)*3+1; return string.byte(s,b,b+2) end
local function music_tick()
  lt=lt-1
  if lt<=0 then local n,v,d=gb(LD,lp);sfx(0,n,v,0,d);lt=d;lp=lp%37+1 end
  bt=bt-1
  if bt<=0 then local n,v,d=gb(BS,bp);sfx(1,n,v,1,d);bt=d;bp=bp%50+1 end
end
